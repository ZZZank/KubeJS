package dev.latvian.kubejs.util.time;
...
public interface TimeJS {
	...
	static TemporalAmount wrapTemporalAmount(Object o) {
		if (o instanceof TemporalAmount d) {
			...
			val matcher = TEMPORAL_AMOUNT_PATTERN.matcher(o.toString());
			...
			while (matcher.find()) {
				val amount = Double.parseDouble(matcher.group(1));
